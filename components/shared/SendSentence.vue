<template>
  <transition>
    <div v-show="getShow" class="sentence-box" :class="{held: this.held, boxHeightInPosts: getMode === 'reply'}">
  ...
  export default{
    ...
    methods:{
      ...
      async setReplyTweetId(id) {
        await this.$axios.$patch(
          'https://q-box.microcms.io/api/v1/q_box_posts/' + this.getContentOriginId, 
          {
            replyTweetId: id
          }, 
          {
  ...
  @media (max-width:520px) {
    ...
    .v{
      ...
      &-leave{
        ...
        &-active{
          transition: opacity .1s, padding-top .5s, height .5s;
        }
      }
    }
  }
