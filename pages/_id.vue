<template>
  <div id="app">
    <header>
      ...
      <h1>お手サーの質問箱</h1>
      <input type="text" placeholder="語句で検索" v-model="qWord" @keyup.enter="searchPost">
    </header>
    <div id="id-container" v-show="showIdContainer">
      <ul>
        <li v-for="post in posts" :key="post.id">
          ...
          <div v-for="reply in post.replies" :key="reply.id" class="secondary-post">
      ...
      if(this.posts) {
        for(const post of this.posts) {
          await this.$axios.$get('https://q-box.microcms.io/api/v1/q_box_replies?filters=replyFor[equals]' + post.id + '[and]replyAnswer[exists]&orders=createdAt',{
            ...
            Common.generateImage(document, response.contents, 'replySentence', '')
